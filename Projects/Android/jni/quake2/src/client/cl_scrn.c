...
==================
*/
void SCR_DrawConsole (float separation)
{
	Con_CheckResize ();
	
	if (cls.state == ca_disconnected || cls.state == ca_connecting)
	{	// forced full screen console
		Con_DrawConsole (1.0,separation);
		return;
	}

	if (cls.state != ca_active || !cl.refresh_prepped)
	{	// connected, but can't render
		Con_DrawConsole (0.5,separation);
	...
	if (scr_con_current)
	{
		Con_DrawConsole (scr_con_current,separation);
	}
	else
	{
...
================
*/
void SCR_ExecuteLayoutString (char *s,float separation)
{
	int		x, y;
	int		value;
	char	*token;
	...
	while (s)
	{
		...
		{	// draw a pic from a stat number
			...
			if (cl.configstrings[CS_IMAGES+value])
			{
				...
				SCR_AddDirtyPoint (x+offset_stereo+23, y+23);
				re.DrawPic (x+offset_stereo, y, cl.configstrings[CS_IMAGES+value]);
			}
			continue;
		}
		...
		{	// draw a deathmatch client block
			...
			DrawAltString (x+offset_stereo+32, y, ci->name);
			DrawString (x+offset_stereo+32, y+8,  "Score: ");
			DrawAltString (x+offset_stereo+32+7*8, y+8,  va("%i", score));
			DrawString (x+offset_stereo+32, y+16, va("Ping:  %i", ping));
			DrawString (x+offset_stereo+32, y+24, va("Time:  %i", time));

			if (!ci->icon)
				ci = &cl.baseclientinfo;
			re.DrawPic (x+offset_stereo, y, ci->iconname);
			continue;
		}
		...
		{	// draw a ctf client block
			...
			if (value == cl.playernum)
				DrawAltString (x+offset_stereo, y, block);
			else
				DrawString (x+offset_stereo, y, block);
			continue;
		}
		...
		{	// draw a pic from a name
			...
			SCR_AddDirtyPoint (x+offset_stereo, y);
			SCR_AddDirtyPoint (x+offset_stereo+23, y+23);
			re.DrawPic (x+offset_stereo, y, token);
			continue;
		}
		...
		{	// draw a number
			...
			token = COM_Parse (&s);
			value = cl.frame.playerstate.stats[atoi(token)];
			SCR_DrawField (x+offset_stereo, y, 0, width, value);
			continue;
		}
		...
		{	// health number
			...
			if (cl.frame.playerstate.stats[STAT_FLASHES] & 1)
				re.DrawPic (x+offset_stereo, y, "field_3");

			SCR_DrawField (x+offset_stereo, y, color, width, value);
			continue;
		}
		...
		if (!strcmp(token, "stat_string"))
		{
			...
			if (index < 0 || index >= MAX_CONFIGSTRINGS)
				Com_Error (ERR_DROP, "Bad stat_string index");
			DrawString (x+offset_stereo, y, cl.configstrings[index]);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawHUDString (token, x+offset_stereo, y, 320, 0);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawString (x+offset_stereo, y, token);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawHUDString (token, x+offset_stereo, y, 320,0x80);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawAltString (x+offset_stereo, y, token);
			continue;
		}
...
================
*/
void SCR_DrawStats (float separation)
{
...
void SCR_DrawLayout (float separation)
{
