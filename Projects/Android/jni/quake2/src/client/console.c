...
================
*/
void Con_DrawConsole (float frac, float separation)
{
	int				i, j, x, y, n;
	int				rows;
	char			*text;
	...
	int offset=0;
	//if (separation>0)offset=viddef.width;


// draw the background
#ifdef QMAX
	re.DrawStretchPic (0+offset, -viddef.height+lines, viddef.width, viddef.height, "conback", 1);
#else
	re.DrawStretchPic (0+offset, -viddef.height+lines, viddef.width, viddef.height, "conback");
#endif
...
#else
		re.DrawChar (offset+viddef.width-44+x*8, lines-12, 128 + version[x] );
#endif
...
// draw from the bottom up
...
#ifdef QMAX
			re.DrawChar (offset+ (x+1)<<3, y, '^', 1);
#else
			re.DrawChar (offset+ (x+1)<<3, y, '^');
#endif			
		y -= 8;
		rows--;
	}
	...
	for (i=0 ; i<rows ; i++, y-=8, row--)
	{
		...
		for (x=0 ; x<con.linewidth ; x++)
#ifdef QMAX
			re.DrawChar (offset+ (x+1)<<3, y, text[x], 1);
#else
			re.DrawChar (offset+ (x+1)<<3, y, text[x]);
#endif
	}
...
#ifdef QMAX
			re.DrawChar ( offset+(i+1)<<3, y, dlbar[i], 1);
#else
			re.DrawChar ( offset+(i+1)<<3, y, dlbar[i]);
#endif
	}
//ZOID
