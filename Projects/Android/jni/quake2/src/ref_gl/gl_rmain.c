...
void R_SetupGL (void)
{
	...
	float iodist=vriodist->value;


	if (gl_state.camera_separation ==0){
	qglViewport (x, y2, w, h);
	}
	else{
	    /*
        if (gl_state.camera_separation>0)
            //qglViewport (x+w, y2, w, h);
            qglViewport (x+w+iodist, y2, w, h);
            //qglViewport (x+w/2, y2, w/2, h);
        else

            qglViewport (x, y2, w, h);
            //qglViewport (x, y2, w/2, h);
	    */
	...
	//
    screenaspect = (float)(r_newrefdef.width/1.0f)/r_newrefdef.height;
...
void R_Register( void )
{
	...
	vriodist= ri.Cvar_Get("vriodist","0.5",CVAR_ARCHIVE);
	...
	ri.Cmd_AddCommand( "inciodist", VRinciodist );
	ri.Cmd_AddCommand( "deciodist", VRdeciodist );
}
